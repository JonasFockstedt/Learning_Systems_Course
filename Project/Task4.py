...
grid_searches = dict()
cv_scores = dict()
classification_models = {'KNeighborsClassifier': KNeighborsClassifier(), 'SVC': SVC(), 'KMeans': KMeans(), 
                        'LogisticRegression': LogisticRegression(),'DecisionTreeClassifier': DecisionTreeClassifier(), 
                        'MLPClassifier': MLPClassifier()}
acc_scores = {'KNeighborsClassifier': [], 'SVC': [], 'KMeans': [], 'LogisticRegression': [], 
            'DecisionTreeClassifier': [], 'MLPClassifier': []}
standard_deviation_scores = {'KNeighborsClassifier': [], 'SVC': [], 'KMeans': [], 'LogisticRegression': [], 
            'DecisionTreeClassifier': [], 'MLPClassifier': []}
def findOptimalNumberOfFeatures():
    ...
    for model in classification_models.keys():
        ...
        for number_of_components in range(1, Xtrain.shape[1] + 1):
            ...
            score = 100*cross_val_score(classification_models[model], Xtrain_reduced, Ytrain.ravel(),
