...
grid_searches = dict()
cv_scores = dict()
acc_scores = {'KNeighborsClassifier': [], 'DecisionTreeClassifier': [], 'RandomForestClassifier': [], 'SVC': [], 'LogisticRegression': [], 'MLPClassifier': []}
standard_deviation_scores = {'KNeighborsClassifier': [], 'DecisionTreeClassifier': [], 'RandomForestClassifier': [], 'SVC': [], 'LogisticRegression': [], 'MLPClassifier': []}
...
def findOptimalFeatures():
    ...
    for model in classification_models.keys():
        print(f'Finding optimal number of features for {type(classification_models[model]).__name__} model...')
        # Things to set up for every model.
        kf_10 = KFold(n_splits=10, shuffle=True, random_state=42)
        optimal_PCA_numbers.update({type(classification_models[model]).__name__: 0})
...
def parameterTuning():
    parametersKNN = {'n_neighbors': np.arange(1,10,1), 'weights': ('uniform', 'distance'), 'p': (1, 2)}
    parametersDT = {'criterion': ('gini', 'entropy'), 'splitter': ('best', 'random')}
    parametersRF = {'n_estimators': np.arange(50, 200, 50), 'criterion': ('gini', 'entropy')}
    parametersSVC = {'C': np.arange(1, 15, 1), 'kernel': ('linear', 'poly'), 'gamma': ('scale', 'auto')}
    parametersLR = {'penalty': ('l1', 'l2'), 'C': np.arange(1, 10, 1), 'solver': ('lbfgs', 'liblinear')}
    parametersMLP = {'solver': ('lbfgs', 'sgd', 'adam'), 'alpha': np.arange(0.00005, 0.001, 0.0005)}

    parameters = {'KNeighborsClassifier': parametersKNN, 'DecisionTreeClassifier': parametersDT, 'RandomForestClassifier': parametersRF, 'SVC': parametersSVC, 'LogisticRegression': parametersLR, 'MLPClassifier': parametersMLP}
    for model in classification_models.keys():
        print(f'Tuning the {model} model...')
        # Perform grid search.
        classifier = GridSearchCV(classification_models[model], parameters[model], scoring='accuracy', n_jobs=-1, cv=10)
        ...
        cv_scores.update({model: classifier.best_score_})
...
def trainBestModel():
    ...
    kf_10 = KFold(n_splits=10, shuffle=True, random_state=42)
    # Run 10-fold cross validation.
    score = 100*cross_val_score(best_model, X_val_normalized, Y_val.ravel(), cv=kf_10, scoring='accuracy', n_jobs=-1).mean()
    print(f'Average validation score of the {type(best_model).__name__} model: {score} (accuracy %).')
